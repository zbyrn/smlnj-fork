...
functor Convert (MachSpec : MACH_SPEC) : CONVERT =
  struct
    ...
    val mkvn = LV.namedLvar o Symbol.varSymbol
    val cplv = LV.dupLvar
	 ...
	 fun lpvars vl =
	   let fun h([], z) = rev z
	    ...
	    in h(vl, [])
         ...
         fun validateFailure name =
           let val (debug,data,hdlr) = (mkv(), mkv(), mkv())
           in  RECORD(RK_RECORD,
                 [(STRING "<auto-generated>", OFFp0),
                  (tagInt 0, OFFp0)], debug,
               RECORD(RK_RECORD,
                 [(VAR name, OFFp0), (tagInt 0, OFFp0),
                  (VAR debug, OFFp0)], data,
               LOOKER(P.GETHDLR, [], hdlr, FUNt,
               APP(VAR hdlr, [VAR bogus_cont, VAR data]))))
           end
...
(* FIXME: we should use continuations, not functions, as handlers *)
    ...
    (*            PURE(P.WRAP, [lpvar u], v, FUNt, c(VAR v))          *)
                 ...
                 *       fix k'(x) =
                 *         getvar() -> m''
                 *         m''(bogus_cont, tok, x) // note that we are discarding [[e]]
                 *       f(k', token)
                 *)
	      | F.PRIMOP((_,AP.RESET,_,_), [f], v, e) =>
                  let val (tok, tok') = (mkvn "tok", mkvn "tok")
                      val (m, m', m'') = (mkvn "meta", mkvn "meta", mkvn "meta")
                      val (k, k') = (mkvn "bogk", mkvn "k")
                      val x = mkv()
                      val metacont =
                        (ESCAPE, m', [k, tok', v], [bogCNTt, CU.BOGt, CU.BOGt],
                          BRANCH (P.PEQL, [VAR tok, VAR tok'], mkv(),
                            (* true *)
                            SETTER (P.SETVAR, [VAR m], loop(e, c)),
                            (* false *)
                            validateFailure tok'))
                      val cont =
                        (CONT, k', [x], [CU.BOGt],
                          LOOKER(P.GETVAR, [], m'', FUNt,
                          APP(VAR m'', [VAR bogus_cont, VAR tok, VAR x])))
                  in  PURE(P.MAKEREF, [STRING "Context"], tok, CU.BOGt,
                      LOOKER(P.GETVAR, [], m, FUNt,
                      FIX([metacont],
                      SETTER(P.SETVAR, [VAR m'],
                      FIX([cont],
                      APP(lpvar f, [VAR k', VAR tok]))))))
                  end
                (*  [[ let v = shift (tok, f) in e ]] c
                 *     ==>
                 *       fix h(k', v) =
                 *         getvar() -> m'
                 *         fix m''(_, tok', w) =
                 ...
                 *       f(k, h)
                 *)
	      | F.PRIMOP((_,AP.SHIFT,_,_), [tok, f], v, e) =>
                  let val (k, k') = (mkvn "k", mkvn "k")
                      val h = mkvn "h"
                      val (m, m', m'') = (mkvn "m", mkvn "m", mkvn "m")
                      val (tok, tok') = (lpvar tok, mkvn "tok")
                      val bogk = mkvn "bogk"
                      val (x, w) = (mkv(), mkv())
                      val metacont =
                        (ESCAPE, m'', [bogk, tok', w], [bogCNTt, CU.BOGt, CU.BOGt],
                          BRANCH (P.PEQL, [tok, VAR tok'], mkv(),
                            (* true *)
                            SETTER(P.SETVAR, [VAR m'], APP(VAR k', [VAR w])),
                            (* false *)
                            validateFailure tok'))
                      val argfun =
                        (ESCAPE, h, [k', v], [CNTt [CU.BOGt], CU.BOGt],
                          LOOKER(P.GETVAR, [], m', FUNt,
                            FIX([metacont],
                              SETTER(P.SETVAR, [VAR m''],
                                loop(e, c)))))
                      val cont =
                        (CONT, k, [x], [CU.BOGt],
                          LOOKER(P.GETVAR, [], m, FUNt,
                            APP(VAR m, [VAR bogus_cont, tok, VAR x])))
                  in  FIX ([argfun], FIX ([cont],
                        APP (lpvar f, [VAR k, VAR h])))
                  end
